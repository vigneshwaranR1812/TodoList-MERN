...
const Due = () => {
  ...
  useEffect(() => {
    ...
    async function fetchAllTask() {
      ...
      const res = await axios.get(
        "http://localhost:5000/api/v1/task/getAllDeadLineTask",
        {
          headers,
        }
      );
  ...
  return (
    <>
      {" "}
      <h1 className="text-center p-7 text-3xl">Due Tasks</h1>
      <div className="w-[85%]  m-auto flex flex-wrap justify-center gap-4 p-4">
        {allTask.length === 0 ? (
          <h1 className="text-center mt-10 text-2xl">No Due Task </h1>
        ) : (
          allTask.map((task) => {
            return (
              <div
